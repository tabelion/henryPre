...
function tienePropiedad (objeto, propiedad) {
  ...
  // cuyo nombre es igual al valor del argumento "propiedad" "propiedad" es un string
  // De lo contrario, devuelve "false"
  // Tu código:
  let canti = Object.keys(objeto).length;
  var nombre;
  for (let i=0; i<canti; i++){
    nombre = Object.keys(objeto)[i];
    if (nombre === propiedad) {
      return true;
    }
  }
...
function verificarPassword (usuario, password) {
  ...
  // Tu código: ok
  var repuesta = false;
  if (usuario.password === password) {repuesta = true}
...
function pasarUsuarioAPremium (usuarios) {
  ...
  // Tu código:
  let canti = Object.keys(usuarios).length;
